...
| **The published site still uses Lovable's backend**  | **Closed 3 Aug 2026 — REOPENED 6 Aug 2026.** PR #32 repointed `.env`, but the premise — "`.env` is the only thing that decides which database the app reaches" — did not hold for the **published artifact**: Lovable builds the site and supplies its own backend variables, so the bundle served at `neuvto.com` resolved to `vkyvzhgigncranprhidn` while the repoint sat merged and inert. Closed on 3 Aug by asking Lovable to publish `main` (commit `68094bc`), verified from the bundle: `/assets/client-B-ugLY2U.js` contained `udrzhfgwqgolvyimbwto.supabase.co` and no other ref. **It recurred on 6 Aug**, same project ref, same mechanism — that same good chunk was replaced by `client-D0Ad218J.js` pointing at pre-prod, and sign-in returned HTTP 200 while sending nothing, because it was authenticating against the wrong database. The re-check written here was not run: the deploy was verified by grepping for two feature strings, both of which were present and correct. **A paragraph in a document is not a check.** It is now `scripts/verify-deploy.sh`, which walks the whole import graph (the ref lives in a vendor chunk no route's HTML mentions), refuses to pass when it finds no ref at all, and fails when two routes are served by different builds — all three of which happened on 6 Aug. Run it after every Lovable deploy, before believing anything about the deploy. | **Open** — awaiting a Lovable publish that resolves to `udrzhfgwqgolvyimbwto` |
