...
**Deactivation is not yet a revocation of access.** See known gaps.
...
| **A deactivated person keeps access**                | `current_org_id()` checks `deleted_at` but not `is_active`, so somebody deactivated keeps a working session — demonstrated by submitting a leave request after being deactivated. Deactivation currently means "cannot be resolved as an approver", not "cannot use the product". D20 says sessions should be revoked. **Deliberately not fixed in step 11: there is no reactivation path, so revoking access before building one turns a mis-click into an unrecoverable lockout.** The two belong in the same step. | Next, with reactivation          |
