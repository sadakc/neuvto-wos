...
| **The published site still uses Lovable's backend**  | PR #32 repointed `.env` to `neuvto-wos-prod` and its premise — "`.env` is the only thing that decides which database the app reaches" — does not hold for the **published** artifact. The bundle served at `neuvto.com` on 3 Aug 2026 resolves to `vkyvzhgigncranprhidn` and mentions our own project nowhere: Lovable builds the site and supplies its own backend variables. The repoint is merged but not live, so anything configured on `neuvto-wos-prod` — including the auth fixes above — changes nothing a customer experiences until the site is rebuilt against it. `neuvto-wos-prod` holds all migrations and **zero rows**.                                                               | **Before any customer signs in** |
