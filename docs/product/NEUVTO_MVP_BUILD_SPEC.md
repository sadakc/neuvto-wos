...
| ~~**The published site still uses Lovable's backend**~~ | **Closed 3 Aug 2026.** PR #32 repointed `.env`, but the premise — "`.env` is the only thing that decides which database the app reaches" — did not hold for the **published artifact**: Lovable builds the site and supplied its own backend variables, so the bundle served at `neuvto.com` resolved to `vkyvzhgigncranprhidn` while the repoint sat merged and inert. Closed by asking Lovable to publish `main` (commit `68094bc`). Verified from the deployed bundle rather than from the deploy succeeding: `/assets/client-B-ugLY2U.js` served from `neuvto.com` contains `udrzhfgwqgolvyimbwto.supabase.co` and no other project ref. **How to re-check after any Lovable deploy:** fetch a page, take the `modulepreload` chunk list, and grep the client chunk for `.supabase.co` — the app shell itself names no project, so nothing about the page will tell you. | Done                             |
