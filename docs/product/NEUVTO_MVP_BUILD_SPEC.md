...
| **Sign-in email is not presentable**                 | Confirmed live 31 Jul 2026: it arrives from `no-reply@auth.lovable.cloud`, and sends a link where the screen asks for a code. BOTH the Magic Link and Confirm signup templates need `{{ .Token }}` — a new signup gets the latter. Needs custom SMTP on Resend plus two template edits, all in Lovable's backend settings. See `docs/operations/EMAIL_AND_DOMAINS.md`. | **Before any customer signs in** |
