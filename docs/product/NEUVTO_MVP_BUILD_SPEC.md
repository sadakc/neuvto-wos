...
| **Approval emails name a database column**           | The subject a manager receives is _"Approval needed: leave_request"_, and the requester gets _"Your leave_request request was approved"_. The template substitutes `entity_type` raw, because the platform must not name a module (D30). The fix is for the emitting module to pass a human label in the payload — small, but it changes the approval event's shape, so it is its own step rather than a footnote to this one. Seen by every customer from the first approval onwards. | Before first customer            |
